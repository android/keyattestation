...
class KeyAttestationCertPath(certs: List<X509Certificate>) : CertPath("X.509") {
  ...
  init {
    // Below needed to support parsing software-backed certs.
  ...
  fun provisioningMethod(): ProvisioningMethod {
    return when {
  ...
  // TODO: b/257122887 - Update this to use fields in the RKP root.
  private fun isRemoteProvisioned(): Boolean {
    val rdn = parseDN(this.intermediateCert().subjectX500Principal.getName(X500Principal.RFC1779))
    return rdn["CN"] == "Droid CA2" &&
      rdn["O"] == "Google LLC" &&
      this.certificatesWithAnchor.size > 4 &&
      this.certificatesWithAnchor
        .get(this.certificatesWithAnchor.size - 4)
        .hasProvisioningInfoExtension()
  }
