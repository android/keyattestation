...
class MatchingChallengeTest {

  @Test
  fun checkChallenge_matchingChallenge_returnsTrue() {
    val challengeChecker = MatchingChallenge(ByteString.copyFromUtf8("challenge"))
  ...
  fun verify_expectedChallenge_returnsSuccess() {
    ...
    assertIs<VerificationResult.Success>(
      verifier.verify(chain, MatchingChallenge(ByteString.copyFromUtf8("challenge")))
    )
  }
  ...
  fun verify_unexpectedChallenge_returnsChallengeMismatch() {
    ...
    assertIs<VerificationResult.ChallengeMismatch>(
      verifier.verify(chain, MatchingChallenge(ByteString.copyFromUtf8("foo")))
    )
  }
}
